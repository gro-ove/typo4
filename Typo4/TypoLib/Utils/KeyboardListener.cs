...
namespace TypoLib.Utils {
    /// <summary>
    /// Deals with all hook-related stuff. Pretty basic, but only sets one hook per app just in case. Everything is pretty self-explanatory.
    /// </summary>
    public class KeyboardListener : IDisposable {
        ...
        public static void IgnoreNextReleased(Keys key) {
            var skipRepeaseListener = new KeyboardListener();
            skipRepeaseListener.PreviewKeyUp += (sender, args) => {
                args.Handled = args.Key == key;
                skipRepeaseListener.Dispose();
            };
        }
        #endregion
    }
